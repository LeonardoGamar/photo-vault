...
/// Die Linux-Fassung der nativen macOS-Funktionen war geschrieben, aber nie
/// ausgeführt worden (docs/plan_linux.md, Phasen 2 und 4: „GESCHRIEBEN,
/// UNGETESTET"). Diese Tests rufen sie gegen echte Dateien auf, erzeugt mit
/// denselben Werkzeugen, die auch im Betrieb gebraucht werden.
///
/// Läuft nur unter Linux – anderswo gibt es weder heif-convert noch
/// dcraw_emu, und die Funktionen sind dort auch nicht im Einsatz.
void main() {
  late Directory temp;

  setUp(() {
    temp = Directory.systemTemp.createTempSync('pv_linuxwerkzeuge_');
    LinuxImageTools.vergissWerkzeuge();
  });
  ...
  group('HEIC (Phase 2)', () {
    ...
    ///
    /// Nicht hier erzeugt: Die libheif dieser Maschine kann HEVC lesen, aber
    ...
    final vorlage = File(p.join('test', 'fixtures', 'linux', 'probe.heic'));
    ...
    test('wird gelesen, skaliert und behält seinen Inhalt', () async {
      ...
      final jpeg = await LinuxImageTools.convertToJpeg(heic, maxDimension: 600);
    ...
    }, skip: File(p.join('test', 'fixtures', 'linux', 'probe.heic')).existsSync()
        ? false
        : true);
  });
  ...
  group('RAW (Phase 2)', () {
    ...
    ///
    /// RAW-Dateien sind 10–50 MB gross und je nach Herkunft
    /// unterschiedlich lizenziert – sie gehören nicht in die Historie
    /// (siehe tool/fetch_format_samples.sh im öffentlichen Spiegel, das
    /// sie bei Bedarf lädt). Fehlt die Datei, wird dieser Test
    ...
    test('wird entwickelt, skaliert und lässt das Original in Ruhe', () async {
      ...
      final jpeg = await LinuxImageTools.convertToJpeg(quelle, maxDimension: 800);
  ...
  group('Video (Phase 4)', () {
    File legeVideo(String name, {int sekunden = 4}) {
      ...
      final r = Process.runSync('ffmpeg', [
        ...
        '-c:v', 'libx264', '-pix_fmt', 'yuv420p', datei.path,
      ]);
    ...
    test('Vorschaubild und Länge', () async {
      final video = legeVideo('film.mp4');

      final ergebnis = await LinuxImageTools.videoThumbnail(video, maxDimension: 320);
    ...
    test('Zuschnitt trifft die verlangte Länge', () async {
      final video = legeVideo('lang.mp4', sekunden: 10);
      final ziel = p.join(temp.path, 'kurz.mp4');

      final ok = await LinuxImageTools.trimVideo(video,
      ...
      // oder fast nichts.
      final dauer = await LinuxImageTools.videoDauer(File(ziel));
