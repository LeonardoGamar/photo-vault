...
int timelineColumnsForWidth(double gridWidth) {
  ...
  final count = (availableWidth / (timelineGridMaxCrossAxisExtent + timelineGridSpacing)).ceil();
  return count < 1 ? 1 : count;
}

double timelineRowHeightForWidth(double gridWidth) {
  final availableWidth = gridWidth - timelineGridHorizontalPadding;
  final columns = timelineColumnsForWidth(gridWidth);
...
double timelineMonthGroupHeight(int itemCount, double gridWidth) {
  ...
  return timelineHeaderHeight + rows * timelineRowHeightForWidth(gridWidth);
}
...
double? timelineOffsetForAsset(
  ...
  var offset = 0.0;
  final columns = timelineColumnsForWidth(gridWidth);
  final rowHeight = timelineRowHeightForWidth(gridWidth);
